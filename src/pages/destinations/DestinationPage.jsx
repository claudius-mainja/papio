...
import { FiMapPin, FiClock, FiArrowRight, FiStar, FiPhone, FiMail, FiCalendar } from 'react-icons/fi'
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          <div className="mb-4 inline-flex items-center gap-2 bg-amber-600/20 backdrop-blur-sm rounded-full px-4 py-2 border border-amber-500/30">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">{destination.description}</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid lg:grid-cols-3 gap-12 mb-16">
            <div className="lg:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                {destination.highlights.map((highlight, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-4 bg-amber-50 rounded-xl">
            ...
            <div className="lg:col-span-1">
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-8 border border-amber-100 sticky top-24">
                <h3 className="text-2xl font-bold text-stone-800 mb-6">Quick Info</h3>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-stone-700">{destination.tourCount} Tours Available</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-amber-600 font-bold text-2xl">From</span>
                    <span className="text-amber-600 font-bold text-3xl">${destination.priceFrom}</span>
                  </div>
                </div>
              ...
              {destination.activities.slice(0, 6).map((activityId, idx) => {
                ...
                return (
                  <Link key={idx} to={`/activities/${activity.slug}`} className="group">
                    <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 hover:border-amber-300 transition-all duration-300 hover:-translate-y-1">
            ...
            <div className="mb-16">
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-8 border border-amber-100">
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <FiStar key={i} className={i < destination.testimonial.rating ? 'text-yellow-400' : 'text-gray-300'} />
                  ))}
                </div>
                <p className="text-xl text-stone-700 mb-6 leading-relaxed">"{destination.testimonial.text}"</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-amber-600 flex items-center justify-center text-white font-bold">
                    {destination.testimonial.name.charAt(0)}
                  </div>
                  <div>
                    <p className="font-semibold text-stone-800">{destination.testimonial.name}</p>
                    <p className="text-stone-500 text-sm">{destination.testimonial.location}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-700 rounded-3xl p-12 text-white text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="mailto:info@papioafricaadventures.com?subject=Booking+Inquiry+for+{destination.name}" className="inline-flex items-center justify-center bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-8 rounded-xl transition-all">
